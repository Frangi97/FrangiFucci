			...
			\subsection{Main page}
				...
				%Here a quick description of the functions illustrated above:
					\begin{itemize}
						\item \textbf{App}
						\begin{itemize}
							\item \textbf{forwardLogInRequest()} This method obtain the information from the ``Username" and the
								``Password" fields, and then call the logInManager
							\item \textbf{forwardSignUpRequest} This method display the sign up form, then it obtain all the data
							...
							\item \textbf{forwardMakeReportRequest()} This method display the form to report traffic violations and
								it starts the procedure, it will call all the methods of ``reportManager"
								(will be explained in detail to follow). 
							\item \textbf{fowardEvaluateRequest()} This method display the screen to evaluate traffic violations and
								it starts the procedure, it will call all the methods of ``evaluateManager"
								(will be explained in detail to follow).
							\item \textbf{forwardHistoryrequest()} This method display the history of the user's reported traffic
							...
							\item \textbf{forwardMapRequest()} This method display the map with the streets colored in function of the
								traffic violations' number.
							\item \textbf{forwardRecommendationRequest()} This method will give a suggestion on how to improve the
...
>>>>>>> 062e3b0e4afaf0edc8a9973b1149c0f60bfa88f3
	...
	\item\textbf{R7}: The system must exploit external resources to show maps to the users.
		...
		This is another requirement fulfilled by the mobile application, which will display the map thanks to Google Maps APIs. The streets are shown with different colours, as will be explained in R8.
