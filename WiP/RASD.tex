...
Safestreet is an application to be used both from civilians (users) and authorities, in order to help the latter and reduce traffic violations. Registered authorities can automatically receive reports made by users, so the service acts as an intermediary. The next paragraph gives a more formal description of what is the purpose of the application.
	...
	The main reference document is ``SafeStreets Mandatory Project Assignment'' specification document. In 				chapter 6 the complete list of references is provided.
	...
	\section{Product perspective}
		...
		\begin{figure}[H]
			\begin{subfigure}{\textwidth}
				\includegraphics[scale = 0.55, center]{uml}
				\caption{UML model}
				\label{UML model }
			\end{subfigure}
		\end{figure}
		...
		(respectively that are part of controller and view) and in order to save up space we avoided to write every single getter and setter or attribute.
...
\chapter{Specific Requirments}
	...
	\subsection{World and Machine}
		In this subsection will be explained the World and Machine matrix
		\begin{table}[H]
			\centering
			\begin{tabular}{|c|c|c|}
				\hline
	...
	\section{Design constraints}
		\subsection{Standards compliance}
			The error of the users position is expressed in meters or feet, it depends on the unity preferred by the authorities
			, and has to be less than 10 meters.\\
			The system is designed to protect to the privacy of users' data, as a matter of fact the entire 
			project is subject to the General Data Protection Regulation (GDPR), a regulation in EU 
			law on data protect on and privacy for all individuals within the European Union (EU) and 
			the European Economic Area (EEA).
		\subsection{Hardware limitations}
			As specified in the “Dependencies and constraints” section, hardware requirements are present 
			only in relation to specific functionalities, for example to make report is necessary a device with
			a functioning internet connection, a camera and a gps system.\\
			To recap all the hardware requirements are:
			\begin{itemize}
				\item Any type of internet connection;
				\item a functioning gps system;
				\item a camera.
			\end{itemize}
